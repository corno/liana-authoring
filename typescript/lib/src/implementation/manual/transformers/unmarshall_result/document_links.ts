...
export const Value: Value = ($) => p_.from.state($['unmarshall result']).decide(($): d_out.Links => {
    switch ($[0]) {
        case 'error': return p_.ss($, ($) => p_.literal.list([]))
        case 'success': return p_.ss($, ($) => p_.from.state($).decide(($): d_out.Links => {
            switch ($[0]) {
                ...
                case 'group': return p_.ss($, ($) => p_.from.state($.derived.style).decide(($): d_out.Links => {
                    switch ($[0]) {
                ...
                case 'list': return p_.ss($, ($): d_out.Links => p_.from.list(
                    $.derived.items
                ).flatten(
                    ($) => Value($)
                ))
                ...
                case 'simple': return p_.ss($, ($) => p_.from.state($.definition).decide(($) => {
                    switch ($[0]) {
                        case 'global': return p_.ss($, ($) => p_.from.state($['l entry'].type).decide(($): d_out.Links => {
                            switch ($[0]) {
                ...
                case 'state': return p_.ss($, ($) => p_.from.state($.derived['option status']).decide(($): d_out.Links => {
                    switch ($[0]) {
